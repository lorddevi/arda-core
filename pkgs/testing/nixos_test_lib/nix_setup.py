...
def setup_nix_in_nix(temp_dir: str, closure_info: str | None) -> None:
    ...
    os.environ["ARDA_TEST_STORE"] = f"{temp_dir}/store"
...
def prepare_test_flake(
    temp_dir: str, arda_core_for_checks: str, closure_info: str
) -> Path:
    """Set up Nix store and copy test flake to temporary directory

    Args:
        temp_dir: Temporary directory
        arda_core_for_checks: Path to arda-core-for-checks
    ...
    subprocess.run(["cp", "-r", arda_core_for_checks, flake_dir], check=True)  # noqa: S603, S607
