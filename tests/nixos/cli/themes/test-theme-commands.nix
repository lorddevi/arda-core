...
makeTest {
  ...
  testScript = ''
    ...
    result = machine.succeed("arda theme list")
    print(result)
    ...
    result = machine.succeed("arda --theme nord --help")
    assert "Arda - minimal infrastructure management" in result
    assert "config" in result
    ...
    machine.succeed("arda config --local set theme dracula")
    ...
    result = machine.fail("arda --theme invalid-theme-name --help")
    assert "not found" in result or "invalid" in result.lower()
    ...
    assert "Theme management" in result
